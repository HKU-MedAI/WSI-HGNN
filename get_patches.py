...
def nested_patches(img_slide, out_base, level=(0,), ext='jpeg'):
    ...
    if len(level) == 1:
        patches = glob.glob(os.path.join('WSI_temp_2_files', '*', '*.' + ext))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-j', '--workers', type=int, default=32, help='Number of worker processes to start [4]')
    ...
    parser.add_argument('-b', '--base_mag', type=float, default=40,
    ...
    if len(levels) == 2:
        ...
        out_base = os.path.join('data', 'patches', args.dataset, 'patches_lv0_ps256')
    ...
    for idx, c_slide in enumerate(all_slides):
        ...
        DeepZoomStaticTiler(c_slide, 'WSI_temp_2', levels, args.base_mag, args.objective, args.format, args.tile_size,
                            args.overlap, True, args.quality, args.workers, args.background_t).run()
        nested_patches(c_slide, out_base, levels)
        try:
            shutil.rmtree('WSI_temp_2_files')
