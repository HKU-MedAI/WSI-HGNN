...
def main():
    ...
    parser.add_argument('--lr', default=0.00001, type=float, help='Initial learning rate [0.0002]')
    ...
    parser.add_argument('--dataset', default='COAD', type=str,
                        choices=['Unitopatho', 'COAD', 'BRCA', 'ESCA'], help='Dataset folder name')
    ...
    assert args.dataset in ['Unitopatho', 'COAD', 'BRCA', 'ESCA'], 'Dataset not supported'
    # For Unitopatho, we use one-hot encoding.
    args.num_classes = {'Unitopatho': 6, 'COAD':1, 'BRCA': 1, 'ESCA':1}[args.dataset]

    normal_feats = open(f'{args.data_root}/{args.dataset}/remix_processed/_list_base.txt', 'r').readlines()
    tumor_feats = open(f'{args.data_root}/{args.dataset}/remix_processed/_list_base.txt', 'r').readlines()
    ...
    for t in range(args.num_repeats):
        ...
        if args.num_prototypes is not None:
            # load reduced-bag
            normal_feats_pth = f'{args.data_root}/{args.dataset}/remix_processed/adeno_bag_feats_proto_{args.num_prototypes}_base.npy'
            tumor_feats_pth = f'{args.data_root}/{args.dataset}/remix_processed/squoumos_bag_feats_proto_{args.num_prototypes}_base.npy'
            ...
            if args.mode == 'cov' or args.mode == 'joint':
                ...
                normal_shift_bank_pth = f'{args.data_root}/{args.dataset}/remix_processed/adeno_bag_feats_shift_{args.num_prototypes}_base.npy'
                tumor_shift_bank_pth = f'{args.data_root}/{args.dataset}/remix_processed/squoumos_bag_feats_shift_{args.num_prototypes}_base.npy'
        ...
        acc_fold_list.append(max(accuracy, val_accuracy))
        auc_fold_list.append(max(auc, val_auc))
        f1_fold_list.append(max(test_f1_score, val_f1_score))
