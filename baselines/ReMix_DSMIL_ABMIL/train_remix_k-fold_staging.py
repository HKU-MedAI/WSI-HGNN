...
def convert_label(labels, num_classes=4):
...
def main():
    ...
    args.num_classes = {'Unitopatho': 6, 'COAD':4, 'BRCA': 4}[args.dataset]

    stage_0_feats = open(f'{args.data_root}/{args.dataset}/remix_processed/stage_0_list_base.txt', 'r').readlines()
    stage_1_feats = open(f'{args.data_root}/{args.dataset}/remix_processed/stage_1_list_base.txt', 'r').readlines()
    stage_2_feats = open(f'{args.data_root}/{args.dataset}/remix_processed/stage_2_list_base.txt', 'r').readlines()
    stage_3_feats = open(f'{args.data_root}/{args.dataset}/remix_processed/stage_3_list_base.txt', 'r').readlines()

    kfold_stage_0_feats = np.array_split(stage_0_feats, args.num_repeats)
    kfold_stage_1_feats = np.array_split(stage_1_feats, args.num_repeats)
    kfold_stage_2_feats = np.array_split(stage_2_feats, args.num_repeats)
    kfold_stage_3_feats = np.array_split(stage_3_feats, args.num_repeats)

    kfold_stage_0_index, kfold_stage_1_index, kfold_stage_2_index, kfold_stage_3_index = [], [], [], []
    index_stage_0, index_stage_1, index_stage_2, index_stage_3 = 0, 0, 0, 0

    for x_ in kfold_stage_0_feats:
        kfold_stage_0_index.append([index_stage_0, index_stage_0 + len(x_)])
        index_stage_0 = index_stage_0 + len(x_)
    
    for y_ in kfold_stage_1_feats:
        kfold_stage_1_index.append([index_stage_1, index_stage_1 + len(y_)])
        index_stage_1 = index_stage_1 + len(y_)

    for z_ in kfold_stage_2_feats:
        kfold_stage_2_index.append([index_stage_2, index_stage_2 + len(z_)])
        index_stage_2 = index_stage_2 + len(z_)

    for a_ in kfold_stage_3_feats:
        kfold_stage_3_index.append([index_stage_3, index_stage_3 + len(a_)])
        index_stage_3 = index_stage_3 + len(a_)
    ...
    for t in range(args.num_repeats):
        ...
        if args.num_prototypes is not None:
            # load reduced-bag
            stage_0_feats_pth = f'{args.data_root}/{args.dataset}/remix_processed/stage_0_bag_feats_proto_{args.num_prototypes}_base.npy'
            stage_1_feats_pth = f'{args.data_root}/{args.dataset}/remix_processed/stage_1_bag_feats_proto_{args.num_prototypes}_base.npy'
            stage_2_feats_pth = f'{args.data_root}/{args.dataset}/remix_processed/stage_2_bag_feats_proto_{args.num_prototypes}_base.npy'
            stage_3_feats_pth = f'{args.data_root}/{args.dataset}/remix_processed/stage_3_bag_feats_proto_{args.num_prototypes}_base.npy'
            # loading features
            stage_0_feats_ = np.load(stage_0_feats_pth, allow_pickle=True)
            stage_1_feats_ = np.load(stage_1_feats_pth, allow_pickle=True)
            stage_2_feats_ = np.load(stage_2_feats_pth, allow_pickle=True)
            stage_3_feats_ = np.load(stage_3_feats_pth, allow_pickle=True)

            if args.mode == 'cov' or args.mode == 'joint':
                # loading semantic shift vectors
                stage_0_shift_bank_pth = f'{args.data_root}/{args.dataset}/remix_processed/stage_0_bag_feats_shift_{args.num_prototypes}_base.npy'
                stage_1_shift_bank_pth = f'{args.data_root}/{args.dataset}/remix_processed/stage_1_bag_feats_shift_{args.num_prototypes}_base.npy'
                stage_2_shift_bank_pth = f'{args.data_root}/{args.dataset}/remix_processed/stage_2_bag_feats_shift_{args.num_prototypes}_base.npy'
                stage_3_shift_bank_pth = f'{args.data_root}/{args.dataset}/remix_processed/stage_3_bag_feats_shift_{args.num_prototypes}_base.npy'
                stage_0_semantic_shifts = np.load(f'{stage_0_shift_bank_pth}')
                stage_1_semantic_shifts = np.load(f'{stage_1_shift_bank_pth}')
                stage_2_semantic_shifts = np.load(f'{stage_2_shift_bank_pth}')
                stage_3_semantic_shifts = np.load(f'{stage_3_shift_bank_pth}')
            ...
            for k_fold in range(args.num_repeats):
                if k_fold != t:
                    train_feats.append(stage_0_feats_[kfold_stage_0_index[k_fold][0]:kfold_stage_0_index[k_fold][1]])
                    train_labels.append(np.zeros(len(kfold_stage_0_feats[k_fold]), dtype=np.int32))

                    train_feats.append(stage_1_feats_[kfold_stage_1_index[k_fold][0]:kfold_stage_1_index[k_fold][1]])
                    train_labels.append(np.zeros(len(kfold_stage_1_feats[k_fold]), dtype=np.int32))

                    train_feats.append(stage_2_feats_[kfold_stage_2_index[k_fold][0]:kfold_stage_2_index[k_fold][1]])
                    train_labels.append(np.zeros(len(kfold_stage_2_feats[k_fold]), dtype=np.int32))

                    train_feats.append(stage_3_feats_[kfold_stage_3_index[k_fold][0]:kfold_stage_3_index[k_fold][1]])
                    train_labels.append(np.zeros(len(kfold_stage_3_feats[k_fold]), dtype=np.int32))
                

                    if args.mode == 'joint':
                        semantic_shifts.append(stage_0_semantic_shifts[kfold_stage_0_index[k_fold][0]:kfold_stage_0_index[k_fold][1]])
                        semantic_shifts.append(stage_1_semantic_shifts[kfold_stage_1_index[k_fold][0]:kfold_stage_1_index[k_fold][1]])
                        semantic_shifts.append(stage_2_semantic_shifts[kfold_stage_2_index[k_fold][0]:kfold_stage_2_index[k_fold][1]])
                        semantic_shifts.append(stage_3_semantic_shifts[kfold_stage_3_index[k_fold][0]:kfold_stage_3_index[k_fold][1]])
                      
                else:
                    temp_stage_0 = np.array_split(kfold_stage_0_feats[k_fold], 2)
                    temp_stage_1 = np.array_split(kfold_stage_1_feats[k_fold], 2)
                    temp_stage_2 = np.array_split(kfold_stage_2_feats[k_fold], 2)
                    temp_stage_3 = np.array_split(kfold_stage_3_feats[k_fold], 2)

                    val_feats.append(temp_stage_0[0])
                    val_feats.append(temp_stage_1[0])
                    val_feats.append(temp_stage_2[0])
                    val_feats.append(temp_stage_3[0])
                    test_feats.append(temp_stage_0[1])
                    test_feats.append(temp_stage_1[1])
                    test_feats.append(temp_stage_2[1])
                    test_feats.append(temp_stage_3[1])
