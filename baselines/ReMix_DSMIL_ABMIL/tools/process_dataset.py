...
def process_ESCA():
    dataset_pth = 'datasets/ESCA/'
    graph_path = os.path.join('data/ESCA_kimia_lv0/homogeneous')
    print(len(glob.glob(os.path.join(graph_path ,'*.pkl'))))
    # graph_list = [os.path.basename(x).split('.')[0] for x in glob.glob(os.path.join(graph_path ,'*.pkl'))]
    coad_wsi_list = np.load(os.path.join(dataset_pth, 'real_ESCA_patient_label.pkl'), allow_pickle=True)
    ...
    for token in ['0-normal', '1-tumor']:
        os.makedirs(f'{dataset_pth}/{token}-npy-v2', exist_ok=True)
        class_id = 0 if token == '0-normal' else 1
        wsi_per_class_id_list = [k for k, v in  coad_wsi_list.items() if v == class_id]
        for wsi in wsi_per_class_id_list:
            try:
                print(wsi)
                with open(glob.glob(os.path.join(graph_path, wsi+'*.pkl'))[0],'rb') as f:
                    graph = pickle.load(f)
                    feats = graph.ndata['feat'].numpy() # torch.Size([patches, 1024])
                    np.save(os.path.join(dataset_pth, f'{token}-npy-v2',wsi+'.npy'), feats)
                    if class_id == 0:
                        available_normal_list.append(wsi)
                    else:
                        available_tumor_list.append(wsi)
    ...
    # os.makedirs('datasets/ESCA/remix_processed', exist_ok=True)
    # train_list_txt = open('datasets/ESCA/remix_processed/train_list.txt', 'w')
    # test_list_txt = open('datasets/BRCA/remix_processed/test_list.txt', 'w')
    # train_labels, test_labels = [], []
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='COAD', choices=['Unitopatho', 'COAD', 'BRCA', 'ESCA'])
    ...
    elif args.dataset == 'ESCA':
        print('Convert ESCA datasets (pre-computed features)')
        process_ESCA()
