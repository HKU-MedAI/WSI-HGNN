...
def main():
    ...
    parser.add_argument('--lr', default=0.001, type=float, help='Initial learning rate [0.0002]')
    ...
    if args.dataset == 'ESCA':
        normal_path = pd.read_csv('datasets/ESCA/list/adeno_list_base.csv').iloc[0:,:]
        tumor_path = pd.read_csv('datasets/ESCA/list/squoumos_list_base.csv').iloc[0:,:]

    kfold_normal_path = np.array_split(np.array(normal_path), args.num_fold)
    kfold_tumor_path = np.array_split(np.array(tumor_path), args.num_fold)
    ...
    for i_fold in range(0, args.num_fold):
        ...
        for idx in range(0, args.num_fold):
            if idx != i_fold:
                train_path.append(kfold_normal_path[idx].tolist())
                train_path.append(kfold_tumor_path[idx].tolist())
            else:
                normal_path = np.array_split(kfold_normal_path[idx], 2)
                tumor_path = np.array_split(kfold_tumor_path[idx], 2)
                val_path.append(normal_path[0].tolist())
                val_path.append(tumor_path[0].tolist())
                test_path.append(normal_path[1].tolist())
                test_path.append(tumor_path[1].tolist())
        ...
        for epoch in range(1, args.num_epochs):
            ...
            scheduler.step()
            current_score = (sum(val_aucs) + val_avg_score)/2
        ...
        auc_fold_list.append(sum(aucs)/len(aucs))
