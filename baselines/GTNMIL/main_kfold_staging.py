...
from option_staging import Options
...
all_stage_0_set = args.all_stage_0_set
all_stage_1_set = args.all_stage_1_set
all_stage_2_set = args.all_stage_2_set
all_stage_3_set = args.all_stage_3_set

ids_set_stage_0 = open(all_stage_0_set).readlines()
ids_set_stage_1 = open(all_stage_1_set).readlines()
ids_set_stage_2 = open(all_stage_2_set).readlines()
ids_set_stage_3 = open(all_stage_3_set).readlines()

kfold_ids_set_stage_0 = np.array_split(np.array(ids_set_stage_0), args.num_fold)
kfold_ids_set_stage_1 = np.array_split(np.array(ids_set_stage_1), args.num_fold)
kfold_ids_set_stage_2 = np.array_split(np.array(ids_set_stage_2), args.num_fold)
kfold_ids_set_stage_3 = np.array_split(np.array(ids_set_stage_3), args.num_fold)
...
for i_fold in range(0, args.num_fold):
    ...
    for idx_fold in range(0, args.num_fold):
        if idx_fold != i_fold:
            ...
            ids_train.append(kfold_ids_set_stage_0[idx_fold])
            ids_train.append(kfold_ids_set_stage_1[idx_fold])
            ids_train.append(kfold_ids_set_stage_2[idx_fold])
            ids_train.append(kfold_ids_set_stage_3[idx_fold])
        
        else:
            temp_stage_0 = np.array_split(kfold_ids_set_stage_0[i_fold],2)
            temp_stage_1 = np.array_split(kfold_ids_set_stage_1[i_fold],2)
            temp_stage_2 = np.array_split(kfold_ids_set_stage_2[i_fold],2)
            temp_stage_3 = np.array_split(kfold_ids_set_stage_3[i_fold],2)

            ids_val.append(temp_stage_0[0])
            ids_val.append(temp_stage_1[0])
            ids_val.append(temp_stage_2[0])
            ids_val.append(temp_stage_3[0])

            ids_test.append(temp_stage_0[1])
            ids_test.append(temp_stage_1[1])
            ids_test.append(temp_stage_2[1])
            ids_test.append(temp_stage_3[1])
    ...
    # scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[20,100], gamma=0.1) # gamma=0.3  # 30,90,130 # 0,90,130 -> 150
        ...
        if epoch % 10 == 0:
            ...
            with torch.no_grad():
                ...
                for i_batch, sample_batched in enumerate(dataloader_val):
                    ...
                    preds, labels, _, _ = evaluator.eval_test(sample_batched, model, graphcam)
                    del sample_batched
                    total += int(len(labels.detach().cpu()))
    ...
    with torch.no_grad():
        ...
        for i_batch, sample_batched in enumerate(dataloader_test):
            ...
            out_predictions.append(out.detach().cpu().numpy())
            test_predictions.extend(preds.detach().cpu().numpy())

            del preds, labels, _, out

        test_labels = np.array(test_labels)
        test_predictions = np.array(test_predictions)
        out_predictions =  np.squeeze(np.array(out_predictions), 1)

        auc_value = roc_auc_score(y_true=test_labels, y_score=out_predictions, multi_class='ovr', average='macro')
    ...
    acc_fold_list.append(evaluator.get_scores())
    auc_fold_list.append(auc_value)
    f1_fold_list.append(f1)
