...
def return_auc(possibility_array, label_array):
    label_array = np.array(label_array)
    return roc_auc_score(y_true=label_array, y_score=possibility_array)
...
def main(args):
    ...
    # For ESCA
    zero_list = [x.split('/')[-1] for x in glob.glob('data/ESCA/pyramid/0/*')]
    one_list = [x.split('/')[-1] for x in glob.glob('data/ESCA/pyramid/1/*')]
    all_list = zero_list + one_list
    ...
    for repeat_num_temp in range(repeat_num):
        ...
        for idx_fold in range(0, repeat_num):
            if idx_fold != repeat_num_temp:
                ...
                ids_val.append(temp_normal[1])
                ids_test.append(temp_normal[0])
                ids_val.append(temp_tumor[1])
                ids_test.append(temp_tumor[0])
...
def get_params():
    ...
    parser.add_argument("--all_data_path", type=str, default="data/h2graph_data/kimia_aug_combine_ESCA_10x20x_512pixel_tree_8nb_gnn_data.pkl", help="Path of training data")
    parser.add_argument("--patient_and_label_path", type=str, default="data/h2graph_data/labels/real_ESCA_patient_label.pkl",
    ...
    parser.add_argument("--divide_seed", type=int, default=3, help="Data division seed")
