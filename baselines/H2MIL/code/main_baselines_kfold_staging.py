...
def main(args):
    ...
    ids_set_stage_0, ids_set_stage_1, ids_set_stage_2, ids_set_stage_3 = [], [], [], []

    for i_all_list in all_list:
        if i_all_list in patient_and_label:
            if patient_and_label[i_all_list] == 0:
                ids_set_stage_0.append(i_all_list)
            elif patient_and_label[i_all_list] == 1:
                ids_set_stage_1.append(i_all_list)
            elif patient_and_label[i_all_list] == 2:
                ids_set_stage_2.append(i_all_list)
            else:
                ids_set_stage_3.append(i_all_list)

    kfold_ids_set_stage_0 = np.array_split(np.array(ids_set_stage_0), repeat_num)
    kfold_ids_set_stage_1 = np.array_split(np.array(ids_set_stage_1), repeat_num)
    kfold_ids_set_stage_2 = np.array_split(np.array(ids_set_stage_2), repeat_num)
    kfold_ids_set_stage_3 = np.array_split(np.array(ids_set_stage_3), repeat_num)
    ...
    for repeat_num_temp in range(repeat_num):
        ...
        for idx_fold in range(0, repeat_num):
            if idx_fold != repeat_num_temp:
                ...
                temp_stage_0 = np.array_split(kfold_ids_set_stage_0[repeat_num_temp],2)
                temp_stage_1 = np.array_split(kfold_ids_set_stage_1[repeat_num_temp],2)
                temp_stage_2 = np.array_split(kfold_ids_set_stage_2[repeat_num_temp],2)
                temp_stage_3 = np.array_split(kfold_ids_set_stage_3[repeat_num_temp],2)
...
def get_params():
    ...
    parser.add_argument("--patient_and_label_path", type=str, default="data/h2graph_data/labels/BRCA_patient_label_staging_v2.pkl",
    ...
    parser.add_argument("--number_of_classes", type=int, default=4, help="number of classes")
